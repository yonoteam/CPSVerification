...
real store \<Rightarrow> bool" ("(_ solvesTheStoreIVP _ withInitState _ )" [70, 70, 70] 68) 
  where "solvesStoreIVP \<phi>\<^sub>S xfList s \<equiv> 
...
real store rel" ("ODEsystem _ with _ " [70, 70] 61) 
  where "ODEsystem xfList with G = 
    {(s,\<phi>\<^sub>S t) |s t \<phi>\<^sub>S. t \<ge> 0 \<and> (\<forall> r \<in> {0..t}. G (\<phi>\<^sub>S r)) \<and> solvesStoreIVP \<phi>\<^sub>S xfList s}"

abbreviation vericond :: "'a pred \<Rightarrow> 'a rel \<Rightarrow> 'a pred \<Rightarrow> bool" ("PRE _ _ POST _")
  where "PRE P X POST Q \<equiv> \<lceil>P\<rceil> \<subseteq> wp X \<lceil>Q\<rceil>"

lemma vericond_gevol: "(PRE P (ODEsystem xfList with G) POST Q) = 
  (\<forall>s. P s \<longrightarrow> (\<forall>s'. (s,s') \<in> (ODEsystem xfList with G) \<longrightarrow> Q s'))"
  unfolding wp_rel by clarsimp
...
lemma wlp_evol_guard:"Id \<subseteq> wp (ODEsystem xfList with G) \<lceil>G\<rceil>"
  by(simp add: rel_aka.fbox_def rel_ad_def guarDiffEqtn_def p2r_def, force)

theorem dWeakening: 
  assumes guardImpliesPost: "\<lceil>G\<rceil> \<subseteq> \<lceil>Q\<rceil>"
  shows "PRE P (ODEsystem xfList with G) POST Q"
  unfolding wp_rel guarDiffEqtn_def using assms by auto
...
theorem dW: "wp (ODEsystem xfList with G) \<lceil>Q\<rceil> = wp (ODEsystem xfList with G) \<lceil>\<lambda>s. G s \<longrightarrow> Q s\<rceil>"
  unfolding rel_aka.fbox_def rel_ad_def guarDiffEqtn_def
...
theorem dCut:
  ...
  shows "PRE P (ODEsystem xfList with G) POST Q"
proof(clarsimp simp: wp_rel)
  fix b y assume "P b" and "(b, y) \<in> ODEsystem xfList with G"
  ...
  from this and pBoxDiffCut have "\<forall> r \<in> {0..t}. C (\<phi>\<^sub>S r)" 
    using \<open>P b\<close> by(clarsimp simp: wp_rel)
  ...
  from this and pBoxCutQ have "\<forall> r \<in> {0..t}. Q (\<phi>\<^sub>S r)" 
    using  boxProgrPred_chrctrztn \<open>P b\<close> by(clarsimp simp: wp_rel)
  thus "Q y" 
    using * by auto 
qed
...
proof(rule_tac f="\<lambda> x. wp x \<lceil>Q\<rceil>" in HOL.arg_cong, safe)
  ...
  from this have "\<forall> r \<in> {0..t}. C (\<phi>\<^sub>S r)" 
    using assms boxProgrPred_chrctrztn
    by (metis (no_types, hide_lams) basic_trans_rules(31) pair_in_Id_conv)
...
theorem dS:
  ...
  unfolding guarDiffEqtn_def rel_aka.fbox_def rel_ad_def
...
lemma conds4vdiffs:
  ...
  shows "\<forall> t \<ge> 0. \<forall> xf \<in> set xfList. (sol s[xfList\<leftarrow>uInput] t) (\<partial> (\<pi>\<^sub>1 xf)) = 
  (\<pi>\<^sub>2 xf) (sol s[xfList\<leftarrow>uInput] t)"
...
proof(clarify, rename_tac t r x f)
  ...
  from this and vars have "((\<lambda>t. F t x) solves_ode (\<lambda>t r. f (F t))) {0..t} UNIV" 
  ...
  have "\<forall> t \<ge> 0. \<forall>r\<in>{0..t}. \<forall> xf \<in> set xfList. (F r (\<partial> (\<pi>\<^sub>1 xf))) = (\<pi>\<^sub>2 xf) (F r)" 
    using assms by auto
...
proof
  ...
  next
    ...
    hence isZero:"\<forall>t\<ge>0.\<forall>r\<in>{0..t}.((\<lambda> t. F t x)has_vector_derivative 0)(at r within {0..t})"
      by blast
...
next
  ...
  \<lbrakk>\<partial>\<^sub>t \<eta>1\<rbrakk>\<^sub>t (F r) \<cdot> \<lbrakk>\<eta>2\<rbrakk>\<^sub>t (F r) + \<lbrakk>\<eta>1\<rbrakk>\<^sub>t (F r) \<cdot> \<lbrakk>\<partial>\<^sub>t \<eta>2\<rbrakk>\<^sub>t (F r)) (at r within {0..t})",simp)
    apply(rule_tac f'1="\<lbrakk>\<partial>\<^sub>t \<eta>1\<rbrakk>\<^sub>t (F r)" and g'1="\<lbrakk>\<partial>\<^sub>t \<eta>2\<rbrakk>\<^sub>t (F r)" in derivative_eq_intros(26))
...
F t (\<partial> x) = \<lbrakk>(map (vdiff \<circ> \<pi>\<^sub>1) xfTail \<otimes> trmTail)\<langle>t\<^sub>V (\<partial> x)\<rangle>\<rbrakk>\<^sub>t (F t)"
  ...
  hence *:"\<lbrakk>(map (vdiff \<circ> \<pi>\<^sub>1) xfTail \<otimes> trmTail)\<langle>Var (\<partial> x)\<rangle>\<rbrakk>\<^sub>t (F t) = F t (\<partial> x)" 
    using tHyp by auto
  ...
  proof(cases "x \<in> set (map \<pi>\<^sub>1 ((y, g) # xfTail))")
    ...
    moreover
    {assume "x = y"
      from this have "((map (vdiff \<circ> \<pi>\<^sub>1) ((y, g) # xfTail)) \<otimes> (\<theta> # trmTail))\<langle>t\<^sub>V (\<partial> x)\<rangle> = \<theta>" 
        by simp
      also from 3 tHyp have "F t (\<partial> y) = g (F t)" 
        by simp
      moreover from 2 have "\<lbrakk>\<theta>\<rbrakk>\<^sub>t (F t) = g (F t)" 
        by simp
  ...
  next
    ...
    then have "((map (vdiff \<circ> \<pi>\<^sub>1) ((y, g) # xfTail)) \<otimes> (\<theta> # trmTail)) \<langle>t\<^sub>V (\<partial> x)\<rangle> = t\<^sub>V (\<partial> x)" 
      using substList_cross_vdiff_on_non_ocurring_var 
...
proof(clarify)
  ...
  from this obtain t::"real" and F::"real \<Rightarrow> real store" 
    ...
    using guarDiffEqtn_def by auto
  then have "\<forall>x. x \<notin> varDiffs \<longrightarrow> F 0 x = a x" 
    using solves_store_ivpD(6) by blast
  from this have "\<lbrakk>\<eta>\<rbrakk>\<^sub>t a = \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F 0)" 
    using termVarsHyp eqInVars_impl_eqInTrms by blast
  hence obs1:"\<lbrakk>\<eta>\<rbrakk>\<^sub>t (F 0) = 0" 
    using aHyp by simp
  hence obs2:"\<forall>r\<in>{0..t}. ((\<lambda>s. \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F s)) has_vector_derivative \<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r)) (at r within {0..t})" 
  ...
  ultimately have "\<forall>r\<in>{0..t}. ((\<lambda>s. \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F s)) has_vector_derivative 0) (at r within {0..t})" 
    using obs2 by auto
  hence "\<forall>s\<in>{0..t}. ((\<lambda>x. \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F x)) has_derivative (\<lambda>x. x *\<^sub>R 0)) (at s within {0..t})" 
    using tcHyp by (metis has_vector_derivative_def)
  ...
  then show "\<lbrakk>\<eta>\<rbrakk>\<^sub>t c = 0" 
...
\<lbrakk>((map (vdiff \<circ> \<pi>\<^sub>1) xfList) \<otimes> uInput) \<langle>\<partial>\<^sub>t \<eta>\<rangle>\<rbrakk>\<^sub>t st = 0"
  ...
  from this have aHyp:"a = b \<and> \<lbrakk>\<eta>\<rbrakk>\<^sub>t a = 0"
    by (simp add: p2r_def)
  ...
  from this and aHyp have "\<forall> c. (a,c) \<in> (ODEsystem xfList with G) \<longrightarrow> \<lbrakk>\<eta>\<rbrakk>\<^sub>t c = 0" 
    by blast
...
\<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r)) (at r within {0..t})" 
  ...
  ultimately have *:"\<forall>r\<in>{0..t}. \<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r) \<ge> 0" 
    by simp
  from obs2 and tcHyp have "\<forall>r\<in>{0..t}. ((\<lambda>s. \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F s)) has_derivative 
(\<lambda>x. x *\<^sub>R (\<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r)))) (at r within {0..t})" 
...
\<and> \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F t) - \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F 0) = t \<cdot> (\<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r))" 
  ...
  thus "\<lbrakk>\<eta>\<rbrakk>\<^sub>t c > 0"
    using obs1 tcHyp by (smt mult_nonneg_nonneg) 
next
  ...
  proof(clarify)
    ...
    from this have "\<lbrakk>\<eta>\<rbrakk>\<^sub>t a = \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F 0)" 
      using termVarsHyp eqInVars_impl_eqInTrms by blast
    hence obs1:"\<lbrakk>\<eta>\<rbrakk>\<^sub>t (F 0) \<ge> 0" 
      using aHyp tcHyp by simp
    hence obs2:"\<forall>r\<in>{0..t}. ((\<lambda>s. \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F s)) has_vector_derivative \<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r)) (at r within {0..t})" 
      using tcHyp derivationLemma termVarsHyp by blast
    have "(\<forall>t\<ge>0. \<forall> xf \<in> set xfList. F t (\<partial> (\<pi>\<^sub>1 xf)) = \<pi>\<^sub>2 xf (F t))"
    ...
    ultimately have *:"\<forall>r\<in>{0..t}. \<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r) \<ge> 0" 
      by simp
    have "\<forall>r\<in>{0..t}. ((\<lambda>s. \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F s)) has_derivative (\<lambda>x. x *\<^sub>R (\<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r)))) (at r within {0..t})" 
      using obs2 tcHyp by (simp add: has_vector_derivative_def) 
    hence "\<exists>r\<in>{0..t}. \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F t) - \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F 0) = t \<cdot> (\<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r))" 
      using mvt_very_simple and tcHyp by fastforce
    then obtain r where "\<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r) \<ge> 0 \<and> 0 \<le> r \<and> r \<le> t \<and> \<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F t) \<ge> 0
\<and> \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F t) - \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F 0) = t \<cdot> (\<lbrakk>\<partial>\<^sub>t \<eta>\<rbrakk>\<^sub>t (F r))" 
...
proof(clarify)
  ...
  from this obtain t::"real" and F::"real \<Rightarrow> real store" 
    where tcHyp:"t\<ge>0 \<and> F t = c \<and> solvesStoreIVP F xfList a" 
    using guarDiffEqtn_def by auto 
...
\<lbrakk>((map (vdiff \<circ> \<pi>\<^sub>1) xfList) \<otimes> uInput)\<restriction>\<partial>\<^sub>P (\<theta> \<doteq> \<eta>)\<restriction>\<rbrakk>\<^sub>P st" 
      by blast
...
\<lbrakk>((map (vdiff \<circ> \<pi>\<^sub>1) xfList) \<otimes> uInput)\<langle>\<partial>\<^sub>t (\<theta> \<oplus> (\<ominus> \<eta>))\<rangle>\<rbrakk>\<^sub>t st = 0" 
      by simp
    also have "trmVars (\<theta> \<oplus> (\<ominus> \<eta>)) \<subseteq> UNIV - varDiffs" using Eq.prems(2) 
      by simp
    moreover have "\<lbrakk>\<theta> \<oplus> (\<ominus> \<eta>)\<rbrakk>\<^sub>t a = 0" using Eq.prems(1) 
      by simp
    ...
    hence "\<lbrakk>\<theta> \<oplus> (\<ominus> \<eta>)\<rbrakk>\<^sub>t (F t) = 0" 
      using tcHyp cHyp by simp
    from this have "\<lbrakk>\<theta>\<rbrakk>\<^sub>t (F t) = \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F t)" 
      by simp
    also have "(\<lbrakk>\<theta> \<doteq> \<eta>\<rbrakk>\<^sub>P) c = (\<lbrakk>\<theta>\<rbrakk>\<^sub>t (F t) = \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F t))" 
      using tcHyp by simp
...
0 \<le> (\<lbrakk>(map (vdiff \<circ> \<pi>\<^sub>1) xfList \<otimes> uInput)\<langle>\<partial>\<^sub>t (\<eta> \<oplus> (\<ominus> \<theta>))\<rangle>\<rbrakk>\<^sub>t) st" 
    ...
    also from "Less.prems"(2)have "trmVars (\<eta> \<oplus> (\<ominus> \<theta>)) \<subseteq> UNIV - varDiffs" 
      by simp
    moreover have "\<lbrakk>\<eta> \<oplus> (\<ominus> \<theta>)\<rbrakk>\<^sub>t a > 0" 
    ...
    hence "\<lbrakk>\<eta> \<oplus> (\<ominus> \<theta>)\<rbrakk>\<^sub>t (F t) > 0" 
      using tcHyp cHyp by simp
    from this have "\<lbrakk>\<eta>\<rbrakk>\<^sub>t (F t) > \<lbrakk>\<theta>\<rbrakk>\<^sub>t (F t)" 
      by simp
    also have "\<lbrakk>\<theta> \<prec> \<eta>\<rbrakk>\<^sub>P c = (\<lbrakk>\<theta>\<rbrakk>\<^sub>t (F t) < \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F t))" 
      using tcHyp by simp
    ultimately show ?case 
      by simp
  next
    ...
    hence "\<forall>st. G st \<longrightarrow> (\<forall>str. str \<notin> (\<pi>\<^sub>1\<lparr>set xfList\<rparr>) \<longrightarrow> st (\<partial> str) = 0) \<longrightarrow> 
0 \<le> (\<lbrakk>(map (vdiff \<circ> \<pi>\<^sub>1) xfList \<otimes> uInput)\<langle>\<partial>\<^sub>t (\<eta> \<oplus> (\<ominus> \<theta>))\<rangle>\<rbrakk>\<^sub>t) st" 
      using leq_pval_to_tval by metis
    also from "Leq.prems"(2)have "trmVars (\<eta> \<oplus> (\<ominus> \<theta>)) \<subseteq> UNIV - varDiffs" 
      by simp
    moreover have "\<lbrakk>\<eta> \<oplus> (\<ominus> \<theta>)\<rbrakk>\<^sub>t a \<ge> 0" 
      using "Leq.prems"(1) by simp
    ...
    hence "\<lbrakk>\<eta> \<oplus> (\<ominus> \<theta>)\<rbrakk>\<^sub>t (F t) \<ge> 0" 
      using tcHyp cHyp by simp
    from this have "(\<lbrakk>\<eta>\<rbrakk>\<^sub>t (F t) \<ge> \<lbrakk>\<theta>\<rbrakk>\<^sub>t (F t))" 
      by simp
    also have "\<lbrakk>\<theta> \<preceq> \<eta>\<rbrakk>\<^sub>P c = (\<lbrakk>\<theta>\<rbrakk>\<^sub>t (F t) \<le> \<lbrakk>\<eta>\<rbrakk>\<^sub>t (F t))" 
      using tcHyp by simp
    ultimately show ?case 
      by simp
  next
...
proof(clarsimp)
  ...
  from this have aHyp:"a = b \<and> P a" 
    by (simp add: p2r_def)
...
\<lbrakk>((map (vdiff \<circ> \<pi>\<^sub>1) xfList) \<otimes> uInput)\<restriction>\<partial>\<^sub>P \<phi>\<restriction>\<rbrakk>\<^sub>P st"
  ...
  apply(rule_tac C="\<lbrakk>\<phi>\<rbrakk>\<^sub>P" in dCut)
   apply(subgoal_tac "\<lceil>F\<rceil> \<subseteq> wp (ODEsystem xfList with G) \<lceil>F\<rceil>")
