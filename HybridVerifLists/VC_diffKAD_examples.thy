...
lemma motion_with_constant_velocity:
  ...
  apply(rule_tac uInput="[\<lambda> t s. s ''v'' \<cdot> t + s ''x'']" in dSolve_toSolveUBC)
          prefer 9 subgoal by (simp add: wp_rel vdiff_def add_strict_increasing2)
         apply (simp_all add: vdiff_def varDiffs_def)
   prefer 2 apply (simp add: solvesStoreIVP_def vdiff_def varDiffs_def)
  apply (clarify, rule_tac f'1="\<lambda> x. s ''v''" and g'1="\<lambda> x. 0" in derivative_intros(189))
  apply (rule_tac f'1="\<lambda> x.0" and g'1="\<lambda> x.1" in derivative_intros(192))
...
lemma system_where_the_guard_implies_the_postcondition2:
  "PRE (\<lambda> s. s ''x'' = 0)
    (ODEsystem [(''x'',(\<lambda> s. s ''x'' + 1))] with (\<lambda> s. s ''x'' \<ge> 0))
   POST (\<lambda> s. s ''x'' \<ge> 0)"
  apply(simp add: wp_rel)
  by (auto simp: relcomp_def rel_ad_def guarDiffEqtn_def solvesStoreIVP_def)
...
lemma single_hop_ball_and_invariants:
  "PRE (\<lambda> s. 0 \<le> s ''x'' \<and> s ''x'' = H \<and> s ''v'' = 0 \<and> s ''g'' > 0 \<and> 1 \<ge> c \<and> c \<ge> 0)  
      ...
      (IF (\<lambda> s. s ''x'' = 0) THEN (''v'' ::= (\<lambda> s. - c \<cdot> s ''v'')) ELSE (''v'' ::= (\<lambda> s. s ''v''))))
      POST (\<lambda> s. 0 \<le> s ''x'' \<and> s ''x'' \<le> H)"
  apply(simp, rule_tac C = "\<lambda> s.  s ''g'' > 0" in dCut)
...
lemma bouncing_ball:
  "PRE (\<lambda> s. 0 \<le> s ''x'' \<and> s ''x'' = H \<and> s ''v'' = 0 \<and> s ''g'' > 0)  
    ((ODEsystem [(''x'', \<lambda> s. s ''v''),(''v'',\<lambda> s. - s ''g'')] with (\<lambda> s. 0 \<le> s ''x''));
    (IF (\<lambda> s. s ''x'' = 0) THEN (''v'' ::= (\<lambda> s. - s ''v'')) ELSE Id))\<^sup>*
   POST (\<lambda> s. 0 \<le> s ''x'' \<and> s ''x'' \<le> H)"
  apply(rule wp_loopI[of _ "\<lambda>s. 0 \<le> s ''x'' \<and> 0 < s ''g'' \<and> 
    2 \<cdot> s ''g'' \<cdot> s ''x'' = 2 \<cdot> s ''g'' \<cdot> H - (s ''v'' \<cdot> s ''v'')"])
    apply(simp, clarsimp simp: bouncing_ball_invariant, simp)
