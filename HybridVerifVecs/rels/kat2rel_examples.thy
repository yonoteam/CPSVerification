...
lemma circular_motion_invariants: "rel_kat.H 
  \<lceil>\<lambda>s. r\<^sup>2 = (s$(\<restriction>\<^sub>V''x''))\<^sup>2 + (s$(\<restriction>\<^sub>V''y''))\<^sup>2\<rceil> (x\<acute>= C & G) \<lceil>\<lambda>s. r\<^sup>2 = (s$(\<restriction>\<^sub>V''x''))\<^sup>2 + (s$(\<restriction>\<^sub>V''y''))\<^sup>2\<rceil>"
  unfolding sH_diff_inv using circular_motion_invariant by auto

\<comment> \<open>Verified with the flow. \<close>

abbreviation "circular_motion_flow \<tau> s \<equiv> 
  (\<chi> i. if i=\<restriction>\<^sub>V''x'' then s$(\<restriction>\<^sub>V''x'') \<cdot> cos \<tau> + s$(\<restriction>\<^sub>V''y'') \<cdot> sin \<tau>
  else - s$(\<restriction>\<^sub>V''x'') \<cdot> sin \<tau> + s$(\<restriction>\<^sub>V''y'') \<cdot> cos \<tau>)"
...
lemma circular_motion:"rel_kat.H 
  \<lceil>\<lambda>s. r\<^sup>2 = (s$(\<restriction>\<^sub>V''x''))\<^sup>2 + (s$(\<restriction>\<^sub>V''y''))\<^sup>2\<rceil> (x\<acute>= C & G) \<lceil>\<lambda>s. r\<^sup>2 = (s$(\<restriction>\<^sub>V''x''))\<^sup>2 + (s$(\<restriction>\<^sub>V''y''))\<^sup>2\<rceil>"
  by (rule local_flow.sH_g_orbit[OF local_flow_circ_motion]) (auto simp: to_var_inject)
...
abbreviation circular_motion_mtx_flow :: "real \<Rightarrow> real^2 \<Rightarrow> real^2" 
  where "circular_motion_mtx_flow \<tau> s \<equiv> 
  (\<chi> i. if i=(0::2) then s$0 \<cdot> cos \<tau> - s$1 \<cdot> sin \<tau> else s$0 \<cdot> sin \<tau> + s$1 \<cdot> cos \<tau>)"

notation circular_motion_sq_mtx ("C")

notation circular_motion_mtx_flow ("\<phi>\<^sub>C")

lemma circular_motion_mtx_exp_eq: "exp (\<tau> *\<^sub>R C) *\<^sub>V s = \<phi>\<^sub>C \<tau> s"
...
lemma circular_motion_sq_mtx: "rel_kat.H 
  \<lceil>\<lambda>s. r\<^sup>2 = (s$0)\<^sup>2 + (s$1)\<^sup>2\<rceil> (x\<acute>= ((*\<^sub>V) C) & G) \<lceil>\<lambda>s. r\<^sup>2 = (s$0)\<^sup>2 + (s$1)\<^sup>2\<rceil>"
  apply(rule local_flow.sH_g_orbit[OF local_flow_exp])
  unfolding circular_motion_mtx_exp_eq by auto
...
lemma bouncing_ball_invariants:
  ...
  shows "rel_kat.H  
  \<lceil>\<lambda>s. s$(\<restriction>\<^sub>V''x'') = h \<and> s$(\<restriction>\<^sub>V''y'') = 0\<rceil> 
  (((x\<acute>=K g & (\<lambda> s. s$(\<restriction>\<^sub>V''x'') \<ge> 0));
  ...
  apply(rule sH_star[of _ "\<lambda>s. 0\<le>s$(\<restriction>\<^sub>V''x'') \<and> I s"])
  using \<open>h \<ge> 0\<close> apply(simp add: diff_inv)
   apply(rule sH_relcomp[where R="\<lambda>s. 0\<le>s$(\<restriction>\<^sub>V''x'') \<and> I s"])
  ...
  using assms by (subst sH_H, simp_all) (force simp: bb_real_arith)
...
abbreviation "constant_acceleration_kinematics_flow g \<tau> s \<equiv> 
  (\<chi> i. if i=(\<restriction>\<^sub>V ''x'') then g \<cdot> \<tau> ^ 2/2 + s $ (\<restriction>\<^sub>V ''y'') \<cdot> \<tau> + s $ (\<restriction>\<^sub>V ''x'') 
        else g \<cdot> \<tau> + s $ (\<restriction>\<^sub>V ''y''))"
...
lemma bouncing_ball:
  ...
  (IF (\<lambda> s. s$(\<restriction>\<^sub>V''x'') = 0) THEN ((\<restriction>\<^sub>V''y'') ::= (\<lambda>s. - s$(\<restriction>\<^sub>V''y''))) ELSE Id FI))\<^sup>*)
  \<lceil>\<lambda>s. 0 \<le> s$(\<restriction>\<^sub>V''x'') \<and> s$(\<restriction>\<^sub>V''x'') \<le> h\<rceil>"
  apply(rule sH_star[of _ "I"])
  using \<open>h \<ge> 0\<close> apply(simp add: loop_inv)
   apply(rule sH_relcomp[where R="I"])
    apply(rule local_flow.sH_g_orbit[OF local_flow_cnst_acc])
    apply(simp add: loop_inv to_var_inject)
...
lemma const_acc_mtx_powN: "m > 2 \<Longrightarrow> (\<tau> *\<^sub>R K)^m = 0"
