...
lemma single_evolution_ball:
  ...
  shows "rel_kat.H
  \<lceil>\<lambda>s. s $ (\<restriction>\<^sub>V ''x'') = h \<and> s $ (\<restriction>\<^sub>V ''v'') = 0\<rceil> 
  ...
  apply(subst local_flow.sH_g_orbit[OF local_flow_cnst_acc], simp_all)
...
lemma single_evolution_ball_K: "rel_kat.H 
  ...
  apply(subst local_flow.sH_g_orbit[OF local_flow_cnst_acc_matrix], simp_all)
...
lemma circular_motion:"rel_kat.H 
  \<lceil>\<lambda>s. r\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil> (x\<acute>=(*v) C & G) \<lceil>\<lambda>s. r\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil>"
  by (subst local_flow.sH_g_orbit[OF local_flow_circ_matrix]) simp_all
...
lemma bouncing_ball: "rel_kat.H 
  ...
  apply(rule sH_star[of _ "\<lambda>s. 0 \<le> s$0 \<and> 0 > s$2 \<and>  2 \<cdot> s$2 \<cdot> s$0 = 2 \<cdot> s$2 \<cdot> h + (s$1 \<cdot> s$1)"], simp)
   apply(rule sH_relcomp[where R="\<lambda>s. 0 \<le> s$0 \<and> 0 > s$2 \<and>  2 \<cdot> s$2 \<cdot> s$0 = 2 \<cdot> s$2 \<cdot> h + (s$1 \<cdot> s$1)"])
    apply(subst local_flow.sH_g_orbit[OF local_flow_cnst_acc_matrix], simp, simp)
...
next
  case (Suc m)
  assume IH: "2 < m \<Longrightarrow> (\<tau> *\<^sub>R K)^m = 0" and "2 < Suc m"
  then show ?case 
  proof(cases "m \<le> 2")
    case True
    hence "m = 2"
      using \<open>2 < Suc m\<close> le_less_Suc_eq by blast 
    hence "(\<tau> *\<^sub>R K)^(Suc m) = (\<tau> *\<^sub>R K)^3"
      by simp
    also have "... = (\<tau> *\<^sub>R K) \<cdot> (\<tau> *\<^sub>R K)^2"
      by (metis (no_types, lifting) \<open>m = 2\<close> calculation power_class.power.power_Suc)
...
lemma suminf_eq_sum:
  ...
  assumes "\<And>m. m > l \<Longrightarrow> f m = 0"
