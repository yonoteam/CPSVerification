...
lemma circular_motion:
  ...
  shows "\<lceil>\<lambda>s. r\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil> \<le>
   wp ([x\<acute>=(*v) C]{0..t} & G) 
  \<lceil>\<lambda>s. r\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil>"
  apply(subst local_flow.wp_g_orbit[OF local_flow_circ_mtx])
...
lemma bouncing_ball:
  assumes "0 \<le> t" and "t < 1/9" 
  shows "\<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 = h \<and> s $ 1 = 0 \<and> 0 > s $ 2\<rceil> \<subseteq> wp 
  ((([x\<acute>=\<lambda>s. A *v s]{0..t} & (\<lambda> s. s $ 0 \<ge> 0));
...
lemma bouncing_ball_invariants: 
  "\<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 = h \<and> s $ 1 = 0 \<and> 0 > s $ 2\<rceil> \<subseteq> wp 
  ((([x\<acute>=\<lambda>s. A *v s]{0..t} & (\<lambda> s. s $ 0 \<ge> 0));
  (IF (\<lambda> s. s $ 0 = 0) THEN (1 ::= (\<lambda>s. - s $ 1)) ELSE Id FI))\<^sup>*)
  \<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 \<le> h\<rceil>"
