...
lemma bouncing_ball:
  ...
  wp (((x\<acute>=(*v) A & (\<lambda> s. s $ 0 \<ge> 0));
  ...
  2 \<cdot> s$2 \<cdot> s$0 = 2 \<cdot> s$2 \<cdot> h + (s$1 \<cdot> s$1)\<rceil>" in rel_ad_mka_starI)
   ...
   apply(subst local_flow.wp_g_orbit[OF local_flow_cnst_acc_matrix], simp)
   apply(subst wp_trafo) unfolding rel_antidomain_kleene_algebra.cond_def image_le_pred
...
lemma bouncing_ball_invariants:
  ...
  shows "\<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 = h \<and> s $ 1 = 0 \<and> 0 > s $ 2\<rceil> \<subseteq> 
  wp (((x\<acute>=(*v) A & (\<lambda> s. s $ 0 \<ge> 0));
  (IF (\<lambda> s. s $ 0 = 0) THEN (1 ::= (\<lambda>s. - s $ 1)) ELSE Id FI))\<^sup>*)
  \<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 \<le> h\<rceil>"
