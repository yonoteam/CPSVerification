...
lemma bouncing_ball_invariants:
  ...
  apply(simp only: rel_antidomain_kleene_algebra.fbox_seq)
   apply(subst p2r_r2p_wp[symmetric, of "(IF _ THEN _ ELSE skip)"])
  ...
  unfolding rel_antidomain_kleene_algebra.fbox_cond wp_assign 
    rel_antidomain_kleene_algebra.fbox_one apply(rule rel_antidomain_kleene_algebra.fbox_iso) 
  unfolding rel_antidomain_kleene_algebra.ads_d_def rel_ad_def by (force simp: r2p_def p2r_def)  
...
lemma bouncing_ball:
 ...
 apply(rule_tac I="\<lceil>I\<rceil>" in rel_ad_mka_starI)
  ...
  unfolding loop_inv using \<open>g < 0\<close> \<open>h \<ge> 0\<close> by (force simp: bb_real_arith assignD)
...
lemma bouncing_ball_sq_mtx:
  ...
  apply(subst rel_antidomain_kleene_algebra.fbox_cond_var, simp)
   apply(simp add: sq_mtx_vec_prod_eq)
   apply(simp add: p2r_r2p_simps)
  unfolding UNIV_3 apply(simp add: exp_ball_sq_mtx_simps, safe)
  using bb_real_arith(3) apply(force simp: add.commute mult.commute)
  using bb_real_arith(4) by (force simp: add.commute mult.commute)
