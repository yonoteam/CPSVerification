...
lemma wp_cnst_acc:
  assumes "0 \<le> t" and "t < 1"
  shows " wp ([x\<acute>=K g]{0..t} & G) \<lceil>Q\<rceil> = 
    \<lceil>\<lambda> s. \<forall> \<tau> \<in> {0..t}. (G \<rhd> (\<lambda>r. \<phi>\<^sub>K g r s){0--\<tau>}) \<longrightarrow> Q (\<phi>\<^sub>K g \<tau> s)\<rceil>"
  apply(subst local_flow.wp_g_orbit[of "K g" _ 1 "(\<lambda> t x. \<phi>\<^sub>K g t x)"])
  using local_flow_cnst_acc and assms by(auto simp: p2r_def)

lemma single_evolution_ball:
  fixes H::real assumes "0 \<le> t" and "t < 1" and "g < 0"
  shows "\<lceil>\<lambda>s. 0 \<le> s $ (\<restriction>\<^sub>V ''y'') \<and> s $ (\<restriction>\<^sub>V ''y'') = H \<and> s $ (\<restriction>\<^sub>V ''v'') = 0\<rceil> 
  \<le> wp ([x\<acute>=K g]{0..t} & (\<lambda> s. s $ (\<restriction>\<^sub>V ''y'') \<ge> 0))
  \<lceil>\<lambda>s. 0 \<le> s $ (\<restriction>\<^sub>V ''y'') \<and> s $ (\<restriction>\<^sub>V ''y'') \<le> H\<rceil>"
  apply(subst wp_cnst_acc)
...
lemma wp_cnst_acc_matrix:
  assumes "0 \<le> t" and "t < 1/9"
  shows "wp ([x\<acute>=(*v) K]{0..t} & G) \<lceil>Q\<rceil> = \<lceil>\<lambda> s. \<forall>\<tau>\<in>{0..t}. (G \<rhd> (\<lambda>r. \<phi>\<^sub>K r s){0--\<tau>}) \<longrightarrow> Q (\<phi>\<^sub>K \<tau> s)\<rceil>"
  apply(subst local_flow.wp_g_orbit[of "(*v) K" _ "((real CARD(3))\<^sup>2 \<cdot> (\<parallel>K\<parallel>\<^sub>m\<^sub>a\<^sub>x))" "\<phi>\<^sub>K"])
  using local_flow_cnst_acc_matrix and assms by auto

lemma single_evolution_ball_K:
  assumes "0 \<le> t" and "t < 1/9" 
  shows "\<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 = H \<and> s $ 1 = 0 \<and> 0 > s $ 2\<rceil> 
  \<le> wp ([x\<acute>=(*v) K]{0..t} & (\<lambda>s. s $ 0 \<ge> 0)) \<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 \<le> H\<rceil>"
  apply(subst wp_cnst_acc_matrix)
...
lemma circular_motion_invariants:
  assumes "(R::real) > 0"
  shows "\<lceil>\<lambda>s. R\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil> \<le> wp ([x\<acute>=(*v) C]{0..t} & G) \<lceil>\<lambda>s. R\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil>"
  using assms(1) apply(rule_tac C="\<lambda>s. R\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2" in dCut)
...
lemma picard_lindeloef_circ_matrix:
  ...
  apply(rule picard_lindeloef_linear_system)
  unfolding entries_circ_matrix using assms by auto
...
lemma local_flow_circ_mtx: 
  ...
  unfolding local_flow_def local_flow_axioms_def apply safe
  using picard_lindeloef_circ_matrix assms apply blast
   apply(rule has_vderiv_on_vec_lambda)
...
lemma flow_for_Circ_DS:
  assumes "0 \<le> t" and "t < 1/4"
  shows " wp ([x\<acute>=(*v) C]{0..t} & G) \<lceil>Q\<rceil> = 
    \<lceil>\<lambda> x. \<forall> \<tau> \<in> {0..t}. (\<forall>r\<in>{0--\<tau>}. G (\<phi>\<^sub>C r x)) \<longrightarrow> Q (\<phi>\<^sub>C \<tau> x)\<rceil>"
  apply(subst local_flow.wp_g_orbit[of "(*v) C" _ "((real CARD(2))\<^sup>2 \<cdot> (\<parallel>C\<parallel>\<^sub>m\<^sub>a\<^sub>x))" \<phi>\<^sub>C])
  using local_flow_circ_mtx and assms by auto

lemma circular_motion:
  assumes "0 \<le> t" and "t < 1/4" and "R > 0"
  shows "\<lceil>\<lambda>s. R\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil> \<le> wp ([x\<acute>=(*v) C]{0..t} & G) \<lceil>\<lambda>s. R\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil>"
...
lemma bouncing_ball:
  ...
  ((([x\<acute>=\<lambda>s. K *v s]{0..t} & (\<lambda> s. s $ 0 \<ge> 0));
  ...
  2 \<cdot> s $ 2 \<cdot> s $ 0 = 2 \<cdot> s $ 2 \<cdot> H + (s $ 1 \<cdot> s $ 1)\<rceil>"])
   ...
   apply(subst wp_cnst_acc_matrix) using assms apply(simp, simp) apply(subst wp_trafo) 
...
lemma bouncing_ball_invariants: 
  "\<lceil>\<lambda>s. (0::real) \<le> s $ (0::3) \<and> s $ 0 = H \<and> s $ 1 = 0 \<and> 0 > s $ 2\<rceil> \<subseteq> wp 
  ((([x\<acute>=\<lambda>s. K *v s]{0..t} & (\<lambda> s. s $ 0 \<ge> 0));
  (IF (\<lambda> s. s $ 0 = 0) THEN ([1 ::== (\<lambda>s. - s $ 1)]) ELSE Id FI))\<^sup>*)
  \<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 \<le> H\<rceil>"
  ...
  using gravity_invariant apply(blast, force simp: p2r_def, force simp: p2r_def)
  ...
  using energy_conservation_invariant apply(blast, force simp: p2r_def, force simp: p2r_def)
