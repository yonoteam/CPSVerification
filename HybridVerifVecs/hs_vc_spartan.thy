...
lemma kpower_inv: 
  ...
  unfolding kpower_base kpower_simp apply(simp_all add: kcomp_eq, clarsimp) 
...
lemma diff_inv_guard_ignore:
  ...
  using assms unfolding fbox_diff_inv diff_invariant_eq by auto
...
lemma fbox_diff_inv_eq: "diff_invariant I f T S 0 (\<lambda>s. True) = 
  ...
  using init_time apply(clarsimp simp: subset_eq ivp_sols_def)
