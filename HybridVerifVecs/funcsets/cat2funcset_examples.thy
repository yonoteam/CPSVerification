...
text{* Next we use our recently created type to work in a 3-dimensional vector space. We define the
vector field and a flow-candidate for the single-evolution ball on this vector space. Then we follow 
the standard procedure to prove that they are in fact a Lipschitz vector-field and a its flow. *}
...
corollary free_fall_flow_DS:
  ...
  shows "fb\<^sub>\<F> {[x\<acute>=\<lambda>t s. free_fall_kinematics s]{0..t} UNIV @ 0 & G} Q = 
    {x. \<forall> \<tau> \<in> {0..t}. (\<forall>r\<in>{0--\<tau>}. G (free_fall_flow r x)) \<longrightarrow> (free_fall_flow \<tau> x) \<in> Q}"
  apply(subst picard_ivp.ffb_g_orbit[of "\<lambda>t s. free_fall_kinematics s" _ _ 1 _ "(\<lambda> t x. free_fall_flow t x)"])
...
lemma single_evolution_ball:
  assumes "0 \<le> t" and "t < 1" 
  shows "{s. (0::real) \<le> s $ (Abs_three 0) \<and> s $ (Abs_three 0) = H \<and> 
    s $ (Abs_three 1) = 0 \<and> 0 > s $ (Abs_three 2)} 
  \<le> fb\<^sub>\<F> ({[x\<acute>=\<lambda>t s. free_fall_kinematics s]{0..t} UNIV @ 0 & (\<lambda> s. s $ (Abs_three 0) \<ge> 0)})
  {s. 0 \<le> s $ (Abs_three 0) \<and> s $ (Abs_three 0) \<le> H}"
...
that one needs to prove in order to use them for verification in $n$-dimensional vector spaces. *}
...
corollary flow_for_K_DS:
  ...
  shows "fb\<^sub>\<F> {[x\<acute>=\<lambda>t s. K *v s]{0..t} UNIV @ 0 & G} Q = 
    {x. \<forall> \<tau> \<in> {0..t}. (\<forall>r\<in>{0--\<tau>}. G (flow_for_K r x)) \<longrightarrow> (flow_for_K \<tau> x) \<in> Q}"
  apply(subst picard_ivp.ffb_g_orbit[of "\<lambda>t s. K *v s" _ _ "((real CARD(3))\<^sup>2 \<cdot> maxAbs K)" _ 
...
lemma single_evolution_ball_K:
  ...
  \<le> fb\<^sub>\<F> ({[x\<acute>=\<lambda>t s. K *v s]{0..t} UNIV @ 0 & (\<lambda> s. s $ 0 \<ge> 0)})
...
corollary flow_for_Circ_DS:
  ...
  shows "fb\<^sub>\<F> {[x\<acute>=\<lambda>t s. Circ *v s]{0..t} UNIV @ 0 & G} Q = 
    {x. \<forall> \<tau> \<in> {0..t}. (\<forall>r\<in>{0--\<tau>}. G (flow_for_Circ r x)) \<longrightarrow> (flow_for_Circ \<tau> x) \<in> Q}"
  apply(subst picard_ivp.ffb_g_orbit[of "\<lambda>t s. Circ *v s" _ _ "((real CARD(2))\<^sup>2 \<cdot> maxAbs Circ)" _ 
...
((\<lambda>\<tau>. - x \<tau> $ 2) has_derivative (\<lambda>\<tau>. 0)) (at r within {0--\<tau>})"
