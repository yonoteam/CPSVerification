...
proof(unfold diff_invariant_eq, clarsimp)
  ...
  {assume ineq2:"\<nu> (X t) > \<mu> (X t)"
    ...
    ultimately have "continuous_on {t\<^sub>0--t} (\<lambda>\<tau>. \<nu> (X \<tau>))" and "continuous_on {t\<^sub>0--t} (\<lambda>\<tau>. \<mu> (X \<tau>))"
    ...
    hence "\<mu> (X \<tau>) \<noteq> \<nu> (X \<tau>)"
      using dIhyp Xhyp \<open>\<nu> s < \<mu> s\<close> unfolding diff_invariant_eq by force
    hence "False"
...
lemma line_is_local_flow: 
  ...
  apply(rule_tac f'1="\<lambda> s. 0" and g'1="\<lambda> s. c" in derivative_intros(189))
