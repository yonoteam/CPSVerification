...
lemma wp_cnst_acc:
  ...
  using local_flow_cnst_acc and assms by(auto simp: nd_fun_ext)
...
lemma bouncing_ball:
  assumes "0 \<le> t" and "t < 1/9" 
  shows "\<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 = H \<and> s $ 1 = 0 \<and> 0 > s $ 2\<rceil> \<le> wp 
  ((([x\<acute>=(*v) K]{0..t} & (\<lambda> s. s $ 0 \<ge> 0)) \<cdot>
  ...
  apply(rule wp_starI[of _ "\<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> 0 > s $ 2 \<and> 
...
lemma bouncing_ball_invariants: 
  ...
  wp ((([x\<acute>=(\<lambda>s. K *v s)]{0..t} & (\<lambda> s. s $ 0 \<ge> 0)) \<cdot>
  (IF (\<lambda> s. s $ 0 = 0) THEN ([1 ::== (\<lambda>s. - s $ 1)]) ELSE \<eta>\<^sup>\<bullet> FI))\<^sup>\<star>)
  \<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 \<le> H\<rceil>"
  apply(subst star_nd_fun.abs_eq)
