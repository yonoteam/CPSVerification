...
lemma picard_lindeloef_cnst_acc:
  ...
  shows "picard_lindeloef_closed_ivl (\<lambda>t. K g) {0..t} 1 0"
...
term "D (\<lambda>t. \<phi>\<^sub>K g t s) = (\<lambda>t. K g (\<phi>\<^sub>K g t s)) on {0..t}"
...
lemma local_flow_cnst_acc_matrix:
  ...
  shows "local_flow ((*v) A) {0..t} ((real CARD(3))\<^sup>2 \<cdot> (\<parallel>A\<parallel>\<^sub>m\<^sub>a\<^sub>x)) \<phi>\<^sub>A"
  unfolding local_flow_def local_flow_axioms_def apply safe
    apply(rule picard_lindeloef_linear_system[where A=A and t=t])
...
lemma single_evolution_ball_K:
  ...
  \<le> wp ([x\<acute>=(*v) A]{0..t} & (\<lambda>s. s $ 0 \<ge> 0)) 
  \<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 \<le> h\<rceil>"
  apply(subst local_flow.wp_g_orbit[OF local_flow_cnst_acc_matrix])
...
lemma circle_invariant:
  ...
  apply(rule_tac S="{0..t}" in has_vderiv_on_subset)
  by(auto intro!: poly_derivatives simp: matrix_vector_mult_def)

lemma circular_motion_invariants:
  shows "\<lceil>\<lambda>s. r\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil> \<le> 
  wp ([x\<acute>=(*v) C]{0..t} & G) 
...
abbreviation "circular_motion_matrix_flow t s \<equiv> 
  (\<chi> i::2. if i=0 then s$0 \<cdot> cos t - s$1 \<cdot> sin t else s$0 \<cdot> sin t + s$1 \<cdot> cos t)"
...
lemma circular_motion:
  ...
  shows "\<lceil>\<lambda>s. r\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil> \<le>
   wp ([x\<acute>=(*v) C]{0..t} & G) 
  \<lceil>\<lambda>s. r\<^sup>2 = (s $ 0)\<^sup>2 + (s $ 1)\<^sup>2\<rceil>"
  apply(subst local_flow.wp_g_orbit[of "(*v) C" _ "(4 \<cdot> (\<parallel>C\<parallel>\<^sub>m\<^sub>a\<^sub>x))" \<phi>\<^sub>C])
...
lemma bouncing_ball:
  ...
  ((([x\<acute>=(*v) A]{0..t} & (\<lambda> s. s $ 0 \<ge> 0)) \<cdot>
  ...
  2 \<cdot> s $ 2 \<cdot> s $ 0 = 2 \<cdot> s $ 2 \<cdot> h + (s $ 1 \<cdot> s $ 1)\<rceil>"])
   ...
   apply(subst local_flow.wp_g_orbit[of _ _ "9 \<cdot> (\<parallel>A\<parallel>\<^sub>m\<^sub>a\<^sub>x)" \<phi>\<^sub>A])
...
lemma gravity_invariant: "(\<lambda>s. s $ 2 < 0) is_diff_invariant_of (*v) A along {0..t}"
...
lemma energy_conservation_invariant: 
  "(\<lambda>s. 2 \<cdot> s$2 \<cdot> s$0 - 2 \<cdot> s$2 \<cdot> h - s$1 \<cdot> s $ 1 = 0) is_diff_invariant_of (*v) A along {0..t}"
...
lemma bouncing_ball_invariants: 
  ...
  wp ((([x\<acute>=(*v) A]{0..t} & (\<lambda> s. s $ 0 \<ge> 0)) \<cdot>
  (IF (\<lambda> s. s $ 0 = 0) THEN (1 ::= (\<lambda>s. - s $ 1)) ELSE \<eta>\<^sup>\<bullet> FI))\<^sup>\<star>)
  \<lceil>\<lambda>s. 0 \<le> s $ 0 \<and> s $ 0 \<le> h\<rceil>"
  apply(subst star_nd_fun.abs_eq)
