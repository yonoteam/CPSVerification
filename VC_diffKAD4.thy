...
thm solves_ode_def
thm has_vderiv_on_def
lemma "S \<subseteq> T \<Longrightarrow> (f has_vderiv_on f') T 
\<Longrightarrow> (f has_vderiv_on f') S"
by (simp add: ODE_Auxiliarities.has_vderiv_on_subset)
...
abbreviation toInit ::"real store \<Rightarrow> real store" where
...
length (xfList) = length (uInput) \<longrightarrow> (\<forall> t > 0. \<forall> uxf \<in>set (cross_list uInput xfList). 
...
proof(clarify, rename_tac x f xfTail u uTail t s y g)
fix x y::"string" and f g::"real store \<Rightarrow> real" and uTail::"(real \<Rightarrow> real store \<Rightarrow> real)list"
and t::real and xfTail::"(string \<times> (real store \<Rightarrow> real))list" and u s::"real \<Rightarrow> real store \<Rightarrow> real"
...
length xfTail = length uTail \<longrightarrow> (\<forall>t>0. \<forall>uxf\<in>set (cross_list uTail xfTail). 
...
and lengthHyp:"length ((x, f) # xfTail) = length (u # uTail)" and tHyp:"0 < t"
...
{assume eq:"(s, y, g) = (u, x, f)" 
  ...
  (y := s t a, vdiff y := vderiv_of (\<lambda>r. s r a) 
  {0<--< (2 *\<^sub>R t)} t)) y" using tHyp by simp
  also have "((toSol (cross_list uTail xfTail) t a)
  (y := s t a, vdiff y := vderiv_of (\<lambda>r. s r a) 
  {0<--< (2 *\<^sub>R t)} t)) y = s t a" using eq by (simp add: vdiff_noFixPoints)
...
{assume yTailHyp:"(s, y, g) \<in> set (cross_list uTail xfTail)"
  ...
  from 1 and 2 have "?gLHS = toSol (cross_list uTail xfTail) t a y"  by (simp add: tHyp)
  then have "?goal" using 3 by simp}
ultimately show ?goal by blast
qed

lemma toSol_crossList_uxf:
...
lemma conds4InitState:(* toSol respects the initial state. *)
...
and lengthHyp:"length xfList = length uInput"
and varsHyp:"\<forall> xf \<in> set xfList. \<pi>\<^sub>1 xf \<notin> varDiffs"
and solHyp1:"\<forall> st. \<forall>t\<ge>0. \<forall>xf\<in>set xfList. ((\<lambda>t. (toSol (cross_list uInput xfList) t (toInit st)) (\<pi>\<^sub>1 xf)) 
...
u s (toInit a)" using toSol_crossList_uxf by (metis) 
...
lemma conds4RestOfStrings: (* toSol keeps the initial state everywhere else. *)
...
and solHyp:"\<forall> st. \<forall>t\<ge>0. \<forall> xf \<in> set xfList. 
((\<lambda>t. (toSol (cross_list uInput xfList) t (toInit st)) (\<pi>\<^sub>1 xf)) 
has_vderiv_on (\<lambda>t. \<pi>\<^sub>2 xf (toSol (cross_list uInput xfList) t (toInit st)))) {0--t} 
\<and> (\<lambda>t. (toSol (cross_list uInput xfList) t (toInit st)) (\<pi>\<^sub>1 xf)) \<in> {0--t} \<rightarrow> UNIV
\<and> toSol (cross_list uInput xfList) 0 (toInit st) (\<pi>\<^sub>1 xf) = st (\<pi>\<^sub>1 xf)"
...
subgoal using solHyp by(simp add: solves_ivp_def solves_ode_def)
done
...
and solHyp3:"\<forall> st. \<forall>xf\<in>set xfList. toSol (cross_list uInput xfList) 0 (toInit st) (\<pi>\<^sub>1 xf) = st (\<pi>\<^sub>1 xf)"
and uniqHyp:"\<forall> st. \<forall> X. X solvesTheStoreIVP xfList withInitState st andGuard G \<longrightarrow> 
(\<forall> t \<ge> 0. (toSol (cross_list uInput xfList) t (toInit st)) = X t)"
and diffAssgn: "\<forall>st. P st \<longrightarrow> 
(\<forall>t\<ge>0. (\<forall>r \<in> {0<--< t}. G (toSol (cross_list uInput xfList) r (toInit st))) \<longrightarrow> 
...
proof
  ...
  next case True
    ...
    {assume "str \<in> varDiffs"
      ...
      from this Fsolves and sHyp have Fhyp:"\<forall> t\<ge>0. \<forall>r\<in>{0<--<t}. \<forall>xf\<in>set xfList. F r (vdiff (\<pi>\<^sub>1 xf)) = \<pi>\<^sub>2 xf (F r)" 
      using solvesStoreIVP_def xDef by simp
      from strFhyp solHyp and sHyp have "toSol (cross_list uInput xfList) s (toInit a) str = 
      f (toSol (cross_list uInput xfList) s (toInit a))" using solvesStoreIVP_def xDef sorry
      then show ?thesis using Fhyp sHyp eqDerivs strFhyp sorry 
      next
      case False
...
apply(rule_tac uInput="[\<lambda> t s. s ''vel'' \<cdot> t + s ''pos'']" in dSolve_toSolve) (* 9 subgoals *)
subgoal by simp
subgoal by simp
subgoal by (simp add: vdiff_def varDiffs_def)
subgoal by (simp add: vdiff_def)
subgoal by simp
subgoal
  apply(simp add: vdiff_def)
  apply(clarify)
...
subgoal by simp
subgoal by (simp add: vdiff_def)
subgoal sorry (* UNIQUENESS *)
subgoal 
  apply(simp add: wp_trafo vdiff_def, clarify)
