...
text\<open> This chapter adds preliminary lemmas for verification of Hybrid Systems.\<close>
...
notation has_vderiv_on ("(1 \<nabla> _ = (_)/ on _)" [65,65] 61)
...
lemma [poly_derivatives]: "\<nabla> (\<lambda>t. a * t\<^sup>2 / 2) = (*) a on T"
...
lemma [poly_derivatives]: "\<nabla> (\<lambda>t. a * t\<^sup>2 / 2 + v * t + x) = (\<lambda>t. a * t + v) on T"
...
lemma [poly_derivatives]: "\<nabla> (\<lambda>r. a * r + v) = (\<lambda>t. a) on T"
...
lemma [poly_derivatives]: "\<nabla> (\<lambda>t. v * t - a * t\<^sup>2 / 2 + x) = (\<lambda>x. v - a * x) on T"
  apply(subgoal_tac "\<nabla> (\<lambda>t. - a * t\<^sup>2 / 2 + v * t  +x) = (\<lambda>x. - a * x + v) on T", simp)
  by(rule poly_derivatives)

lemma [poly_derivatives]: "\<nabla> (\<lambda>t. v - a * t) = (\<lambda>x. - a) on T"
  apply(subgoal_tac "\<nabla> (\<lambda>t. - a * t + v) = (\<lambda>x. - a) on T", simp)
  by(rule poly_derivatives)
...
lemma has_vderiv_on_vec_lambda:
  ...
  assumes "\<forall>i. \<nabla> (\<lambda>t. x t $ i) = (\<lambda>t. f (x t) $ i) on T"
  shows "\<nabla> x = (\<lambda>t. f (x t)) on T"
...
lemma has_vderiv_on_vec_nth:
  fixes f::"(('a::banach)^('n::finite)) \<Rightarrow> ('a^'n)"
  assumes "\<nabla> x = (\<lambda>t. f (x t)) on T"
  shows "\<nabla> (\<lambda>t. x t $ i) = (\<lambda>t. f (x t) $ i) on T"
  using assms unfolding has_vderiv_on_def has_vector_derivative_def apply clarsimp
...
text\<open> The first locale imposes conditions for applying the Picard-Lindeloef theorem following 
the people who created the Ordinary Differential Equations entry in the AFP. \<close>
...
lemma unique_solution:
  assumes "\<nabla> x = (\<lambda>t. f t (x t)) on T" and "x t\<^sub>0 = s"
    and "\<nabla> y = (\<lambda>t. f t (y t)) on T" and "y t\<^sub>0 = s" and "t \<in> T" 
  shows "x t = y t"
...
lemma fixpoint_solves_ivp:
  shows "\<nabla> (\<lambda>t. phi t s) = (\<lambda>t. f t (phi t s)) on T" and "phi t\<^sub>0 s = s"
...
lemma fixpoint_usolves_ivp:
  assumes "\<nabla> x = (\<lambda>t. f t (x t)) on T" and "x t\<^sub>0 = s" and "t \<in> T"
  shows "x t = phi t s"
  using unique_solution[OF assms(1,2)] fixpoint_solves_ivp assms by blast

end

text\<open> The next locale particularizes the previous one to an initial time equal to 0. Thus making
the function that maps every initial point to its solution a (local) ``flow''. \<close>
...
locale local_flow = picard_lindeloef "(\<lambda> t. f)" T L 0 for f::"('a::banach) \<Rightarrow> 'a" and T L +
  ...
  assumes ivp: "\<nabla> (\<lambda>t. \<phi> t s) = (\<lambda>t. f (\<phi> t s)) on T" "\<phi> 0 s = s "
begin
...
lemma usolves_ivp:
  assumes "\<nabla> x = (\<lambda>t. f (x t)) on T" and "x 0 = s" and "t \<in> T"
  shows "x t = \<phi> t s"
proof-
  have "x t = phi t s" 
...
proof(unfold local_flow_def local_flow_axioms_def, safe)
  ...
  show "\<nabla> (\<lambda>t. \<phi> t s) = (\<lambda>t. f (\<phi> t s)) on T"
...
text\<open> The last locale shows that the function introduced in its predecesor is indeed 
a flow. That is, it is a group action on the additive part of the real numbers.\<close>
locale global_flow = local_flow f UNIV L \<phi> for f L \<phi>
begin 

lemma add_flow_solves: "\<nabla> (\<lambda>\<tau>. \<phi> (\<tau> + t) s) = (\<lambda>\<tau>. f (\<phi> (\<tau> + t) s)) on UNIV"
  apply(subgoal_tac "\<nabla> ((\<lambda>\<tau>. \<phi> \<tau> s) \<circ> (\<lambda>\<tau>. \<tau> + t)) = 
...
proof-
  ...
  have "\<phi> (0 + t2) s = \<phi> t2 s" 
    by simp
  moreover have "\<nabla> (\<lambda>\<tau>. \<phi> (\<tau> + t2) s) = (\<lambda>\<tau>. f (\<phi> (\<tau> + t2) s)) on UNIV" 
...
text\<open> Finally, we exemplify a procedure for introducing pairs of vector fields and their respective 
flows using the previous locales. \<close>

lemma constant_is_picard_lindeloef: "0 \<le> t \<Longrightarrow> picard_lindeloef (\<lambda>t s. c) {0..t} (1 / (t + 1)) 0"
  unfolding picard_lindeloef_def by(simp add: nonempty_set_def lipschitz_on_def, clarsimp, simp)

lemma line_solves_constant: "\<nabla> (\<lambda>\<tau>. x + \<tau> *\<^sub>R c) = (\<lambda>t. c) on {0..t}"
...
lemma line_is_local_flow:
  ...
  using assms constant_is_picard_lindeloef apply blast
