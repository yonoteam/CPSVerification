...
section \<open>Hybrid System Verification\<close>
...
text \<open>The final part corresponds to the finite iteration.\<close>
...
definition "ivp_sols f T t\<^sub>0 s = {x |x. (\<nabla> x = (f \<circ> x) on T) \<and> x t\<^sub>0 = s \<and> t\<^sub>0 \<in> T}"

lemma ivp_solsI: 
  assumes "\<nabla> x = (f \<circ> x) on T" "x t\<^sub>0 = s" "t\<^sub>0 \<in> T"
  shows "x \<in> ivp_sols f T t\<^sub>0 s"
  using assms unfolding ivp_sols_def by blast

lemma ivp_solsD:
  assumes "x \<in> ivp_sols f T t\<^sub>0 s"
  shows "\<nabla> x = (f \<circ> x) on T"
...
lemma g_orbital_eq: "g_orbital f T t\<^sub>0 G s = 
  {x t |t x. t \<in> T \<and> (\<nabla> x = (f \<circ> x) on T) \<and> x t\<^sub>0 = s \<and> t\<^sub>0 \<in> T \<and> G \<rhd> x {t\<^sub>0--t}}"
...
abbreviation g_evol ::"(('a::banach)\<Rightarrow>'a) \<Rightarrow> real set \<Rightarrow> 'a pred \<Rightarrow> 'a \<Rightarrow> 'a set" 
("(1[x\<acute>=_]_ & _)") where "[x\<acute>=f]T & G \<equiv> (\<lambda> s. g_orbital f T 0 G s)"

lemmas g_evol_def = g_orbital_eq[where t\<^sub>0=0]

lemma g_evolI:
  assumes "\<nabla> x = (f \<circ> x) on T" "x 0 = s"
...
lemma g_evolD:
  ...
  and "\<nabla> x = (f \<circ> x) on T" "x 0 = s"
...
proof(rule subset_antisym, simp_all only: subset_eq)
  {fix s' assume "s' \<in> ([x\<acute>=f]T & G) s"
    then obtain x and t where x_ivp:"\<nabla> x = (f \<circ> x) on T" 
...
corollary ffb_line: "0 \<le> t \<Longrightarrow> fb\<^sub>\<F> ([x\<acute>=\<lambda>t. c]{0..t} & G) Q = 
...
proof(rule_tac f="\<lambda> x. fb\<^sub>\<F> x Q" in HOL.arg_cong, rule ext, rule subset_antisym)
  fix s 
  {fix s' assume "s' \<in> ([x\<acute>=f]T & G) s"
    then obtain t::real and x where x_ivp: "\<nabla> x = (f \<circ> x) on T" "x 0 = s"
...
proof(subst ffb_eq, subst g_evol_def, clarsimp)
  fix \<tau>::real and x::"real \<Rightarrow> 'a" assume "(x 0) \<in> P" and "0 \<le> \<tau>" and "\<tau> \<le> t"
    and x_solves:"\<nabla> x = (\<lambda>t. f (x t)) on {0..t}" and guard_x:"(\<forall> r \<in> {0--\<tau>}. G (x r))"
...
lemma DI_necessity:
  ...
  apply(simp add: ffb_eq g_evol_def, clarify)
  oops
...
assumes "\<forall> x. (\<nabla> x = (\<lambda>\<tau>. f (x \<tau>)) on {0..t}) \<longrightarrow> (\<forall> \<tau> \<in> {0..t}. \<forall> r \<in> {0--\<tau>}. 
...
proof(simp add: diff_invariant_def ivp_sols_def, clarsimp)
  fix x \<tau> assume tHyp:"0 \<le> \<tau>" "\<tau> \<le> t" 
    and x_ivp:"\<nabla> x = (\<lambda>\<tau>. f (x \<tau>)) on {0..t}" "\<theta> (x 0) = \<nu> (x 0)" 
...
lemma [ode_invariant_rules]:
fixes \<theta>::"'a::banach \<Rightarrow> real"
assumes "\<forall> x. (\<nabla> x = (\<lambda>\<tau>. f (x \<tau>)) on {0..t}) \<longrightarrow> (\<forall> \<tau> \<in> {0..t}. \<forall> r \<in> {0--\<tau>}. \<theta>' (x r) \<ge> \<nu>' (x r)
...
proof(simp add: diff_invariant_def ivp_sols_def, clarsimp)
  fix x \<tau> assume tHyp:"0 \<le> \<tau>" "\<tau> \<le> t" 
    and x_ivp:"\<nabla> x = (\<lambda>\<tau>. f (x \<tau>)) on {0..t}" "\<nu> (x 0) \<le> \<theta> (x 0)"
...
proof(simp add: diff_invariant_def ivp_sols_def, clarsimp)
  fix x \<tau> assume tHyp:"0 \<le> \<tau>" "\<tau> \<le> t"
    and x_ivp:"\<nabla> x = (\<lambda>\<tau>. f (x \<tau>)) on {0..t}" "\<nu> (x 0) < \<theta> (x 0)"
